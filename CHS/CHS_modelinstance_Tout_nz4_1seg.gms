...
Equations
    ...
    Tout_uncertain_x2(X,T)  ..  Tout('x2',T) =e= Tout_n+0.5*FId*Tout_vertex(T) ;
    Tout_uncertain_x3(X,T)  ..  Tout('x3',T) =e= Tout_n+0.5*FId*Tout_vertex(T) ;
    Tout_uncertain_x4(X,T)  ..  Tout('x4',T) =e= Tout_n+0.5*FId*Tout_vertex(T) ;
    Tout_uncertain_x5(X,T)  ..  Tout('x5',T) =e= Tout_n+0.5*FId*Tout_vertex(T) ;
    Tout_uncertain_x6(X,T)  ..  Tout('x6',T) =e= Tout_n+0.5*FId*Tout_vertex(T) ;
    Tout_uncertain_x7(X,T)  ..  Tout('x7',T) =e= Tout_n+0.5*FId*Tout_vertex(T) ;
    Tout_uncertain_x8(X,T)  ..  Tout('x8',T) =e= Tout_n+0.5*FId*Tout_vertex(T) ;
    Tout_uncertain_x9(X,T)  ..  Tout('x9',T) =e= Tout_n+0.5*FId*Tout_vertex(T) ;
    Tout_uncertain_x10(X,T) ..  Tout('x10',T)=e= Tout_n+0.5*FId*Tout_vertex(T) ;
...
*   For_u(X,T)$(region(X,T)) ..  u(X,T+1) =e= u(X,T-1)+alpha(T)*r1*(u(X-1,T)-2*u(X,T)+u(X+1,T))-v*r2*(u(X+1,T)-u(X-1,T))-c*2*dt*(u(X,T)-k0) ;
    ...
    BC_L1(T)$(ord(T) lt 87).. bc_L(T) =e= bc_L(T+1);
    BC_R1(T)$(ord(T) lt 87).. bc_R(T) =e= bc_R(T+1);
    BC_L2(T)$((ord(T) gt 87) and (ord(T) lt 175)).. bc_L(T) =e= bc_L(T+1);
    BC_R2(T)$((ord(T) gt 87) and (ord(T) lt 175)).. bc_R(T) =e= bc_R(T+1);
    BC_L3(T)$((ord(T) gt 175) and (ord(T) lt 262)).. bc_L(T) =e= bc_L(T+1);
    BC_R3(T)$((ord(T) gt 175) and (ord(T) lt 262)).. bc_R(T) =e= bc_R(T+1);
    BC_L4(T)$((ord(T) gt 262) and (ord(T) lt 350)).. bc_L(T) =e= bc_L(T+1);
    BC_R4(T)$((ord(T) gt 262) and (ord(T) lt 350)).. bc_R(T) =e= bc_R(T+1);

...
*   For_u(X,T)$(region(X,T)) ..  u(X,T+1) =e= u(X,T-1)+alpha(T)*r1*(u(X-1,T)-2*u(X,T)+u(X+1,T))-v*r2*(u(X+1,T)-u(X-1,T))-c*2*dt*(u(X,T)-k0) ;
    ...
    BC_L1(T)$(ord(T) lt 116).. bc_L(T) =e= bc_L(T+1);
    BC_R1(T)$(ord(T) lt 116).. bc_R(T) =e= bc_R(T+1);
    BC_L2(T)$((ord(T) gt 116) and (ord(T) lt 233)).. bc_L(T) =e= bc_L(T+1);
    BC_R2(T)$((ord(T) gt 116) and (ord(T) lt 233)).. bc_R(T) =e= bc_R(T+1);
    BC_L3(T)$((ord(T) gt 233) and (ord(T) lt 350)).. bc_L(T) =e= bc_L(T+1);
    BC_R3(T)$((ord(T) gt 233) and (ord(T) lt 350)).. bc_R(T) =e= bc_R(T+1);

...
*glass envelope
    ...
    I_n(t,x,j)$(ord(t) <= 48) = 400;
    I_n(t,x,j)$(ord(t) > 48 and ord(t) <= 96) = 500;
    I_n(t,x,j)$(ord(t) > 96 and ord(t) <= 144) = 600;
    I_n(t,x,j)$(ord(t) > 144 and ord(t) <= 192) = 500;
    I_n(t,x,j)$(ord(t) > 192 and ord(t) <= 240) = 400;
...
Equations
    fobj
    I_uncertain_x1(t,x,j)
    I_uncertain_x2(t,x,j)
    I_uncertain_x3(t,x,j)
    I_uncertain_x4(t,x,j)
    I_uncertain_x5(t,x,j)
*init    
    eq_init_T_o(t,x,j)
    eq_init_T_a(t,x,j)
    eq_init_T_e(t,x,j)
...
*S,V
    ...
    T_o(t+1,x,j) =e= T_o(t,x,j)+dt/2/(rho_o*cp_o*A_i)*(-m(t)*cp_o*sum(k,A(j,k)*T_o(t  ,x,k))/(dx)+p_i*h_p*(T_a(t  ,x,j)-T_o(t  ,x,j))
                                                       -m(t)*cp_o*sum(k,A(j,k)*T_o(t+1,x,k))/(dx)+p_i*h_p*(T_a(t+1,x,j)-T_o(t+1,x,j)));
    ...
    I_uncertain_x1(t,x,j)  ..  I(t,'1',j) =e= I_n(t,x,j)+300*FId*I_vertex(t) ;
    I_uncertain_x2(t,x,j)  ..  I(t,'2',j) =e= I_n(t,x,j)+300*FId*I_vertex(t) ;
    I_uncertain_x3(t,x,j)  ..  I(t,'3',j) =e= I_n(t,x,j)+300*FId*I_vertex(t) ;
    I_uncertain_x4(t,x,j)  ..  I(t,'4',j) =e= I_n(t,x,j)+300*FId*I_vertex(t) ;
    I_uncertain_x5(t,x,j)  ..  I(t,'5',j) =e= I_n(t,x,j)+300*FId*I_vertex(t) ;
